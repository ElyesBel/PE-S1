...
# %% [markdown]
# Date est de type object ; on aimerait le convertir en type datetime
...
# %% [markdown]
# Le NaN devient NaT (not a number -> not a time)
...
# %%
#df.loc[~df['Date'].isna()] #le ~ permet d'inverser le masque booléen (sélectionne là où c'est False et pas True
...
# %%
# à vous
df.plot(y=cols, title='Valeur de l‘action au cours du temps'); #comme on a déjà set_index, x='Date' par défaut vu qu'il prend l'index par défaut
...
# %%
rolling_28_3 = s.rolling(window=3, center=True).mean()
rolling_28_5 = s.rolling(window=5, center=True).mean()
rolling_28_10 = s.rolling(window=10, center=True).mean()
pd.DataFrame({'original':s, 'roll_3pts':rolling_28_3, 'roll_5pts':rolling_28_5,
              'roll_7pts':rolling_28_7, 'roll_10pts':rolling_28_10}).plot();
...
plt.figure()
plt.title('Valeur haute de juillet 2018 à fin 2019')
df2.plot();
...
# %% [markdown]
# c'est **moins** : en effet on fait des semaines et plus des jours !
...
# %%
# à vous
rolling_an = df2.rolling(window=pd.Timedelta(52, 'W'), center=True).mean()
plt.figure()
plt.title('tutu')
rolling_an.plot();
